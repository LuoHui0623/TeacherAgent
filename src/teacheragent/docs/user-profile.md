...
有些信息不该让用户手写，而应由行为记录算出来：

| 指标 | 来源 | 单位 |
|---|---|---|
| 有效学习时间 | 工作台「开始学习」会话计时（后续功能，见 `Task.md` T10） | h（最小）/ day / month / year |

- 有效学习时间**不设 Markdown 分区**：它是派生指标，随学习会话持续变化，写进画像会立刻过期。
- **计时来源**：后续新增的**工作台（实际首页）**提供「开始学习」按钮，点击即开始计时。不做「事后猜测学习时长」。
- 消费方（如写大纲）需要体量与节奏依据时，直接读派生指标，不依赖画像文本。
...
```jsonc
{
  ...
  "source": {
    "documentKey": "default",
    "markdownVersionId": "admin-20260914123000",
    "contentHash": "sha256:9f2c...",
    "parsedAt": "2026-09-14T12:00:00+08:00"
  },
...
- **`source` 三件套（版本 ID + 内容哈希 + 解析时间）是溯源的锚点**，教材运行时必须带上。
...
> **归属约定**：提示词不通用，就近归属到使用它的能力目录，路径形如 `capabilities/profile/prompts/<名称>.md`。规范见 `docs/prompts.md`。

### 4.1 画像维护提示词

提示词资产：`capabilities/profile/prompts/maintain.md`

职责：把本轮新的学习交互**增量合并**进画像 Markdown。核心约束：累积而非覆盖、只写有依据的结论、水平用档位而非分数、技术栈是累积条目且整合只做归并不丢信息。

### 4.2 画像解析提示词

提示词资产：`capabilities/profile/prompts/parse.md`

职责：把画像 Markdown **整篇**解析为结构化画像。核心约束：「只提取不推断」—— `items[].name` 必须是原文出现过的表述，原文没说的绝不补；分区缺失记入 `warnings` 而不报错。

> 提示词全文以 `capabilities/profile/prompts/` 下的资产为**单一来源**，本文档不再维护副本，避免文档与资产漂移。
### 4.3 大纲能力消费约定

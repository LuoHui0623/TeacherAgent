...
| `store/` | 持久化：`sqlite/`（`database.py` 连接与迁移、`schemas/` 行契约、`repositories/` 仓储、`scripts/` DDL） |
| `config/` | 纯配置：`paths.py` 路径、`env.py` 配置项读取、`llm_catalog.py` 模型候选清单；**不访问 store** |
...
| 模型候选 + 默认项 | `config/llm_catalog.py` |
